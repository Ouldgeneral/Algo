...
def Apropos():
    ...
    in_web=Label(info,text="Version Web",fg='blue',font=font.Font(underline=1) )
    in_web.pack(side=TOP)
    in_web.bind("<Button-1>",lambda e:webbrowser.open_new("https://ouldgeneral.github.io/Algoprojet/Motus.html"))
...
def genere():
    ...
    if(mot==True):
        Cache=random.choice(Mot_cacher).upper()
...
#Button(f,text='Code_Source',command=).pack(side=TOP)
